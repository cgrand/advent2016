...
(def day10-input "bot 119 gives low to bot 18 and high to bot 3
...
(defn day10-1 [input]
  (let [merge #(merge-with into %1 %2)
        init {:bot (reduce merge (for [[_ value bot] (re-seq #"value (\d+) goes to bot (\d+)" input)] {bot [(Long/parseLong value)]}))
              :output {}}
        rules (into {} (for [[_ bot low-type low-id high-type high-id] (re-seq #"bot (\d+) gives low to (bot|output) (\d+) and high to (bot|output) (\d+)" input)]
                         [bot [[(keyword low-type) low-id] [(keyword high-type) high-id]]]))
        step (fn [state]
               (reduce
                 (fn [state [bot [tag id] chip]]
                   (-> state (update :bot dissoc bot) (update tag merge {id [chip]})))
                 state
                 (for [[bot chips] (:bot state)
                      :when (=  2 (count chips))
                      [chip path] (map vector (sort chips) (rules bot))]
                  [bot path chip])))]
    (some #(some (fn [[bot chips]] (when (= (sort chips) [17 61]) bot)) (:bot %)) (iterate step init))))
        ...
        step (fn [state]
               (reduce
                 ...
                 (for [[bot chips] (:bot state)
                      ...
                      [chip path] (map vector (sort chips) (rules bot))]
                  [bot path chip])))]
    (reduce * (mapcat (some #(when-not (seq (:bot %)) (:output %)) (iterate step init)) ["0" "1" "2"]))))